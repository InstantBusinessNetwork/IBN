...
void CResizableDialog::ArrangeLayout()
{
	...
	for (i=0; i<count; ++i)
	{
		...
		if(obj.pPic)
		{
			obj.pPic->GetRect(&objrc);
		}
		else if(obj.pWnd)
		{
			obj.pWnd->GetWindowRect(&objrc);
			ScreenToClient(&objrc);
		}
		else
		{
		...
		if (!newrc.EqualRect(&objrc))
		{
			if (obj.adj_hscroll)
			{
				...
				int max;
				if(obj.pWnd)
					max = obj.pWnd->GetScrollLimit(SB_HORZ);
				else
					max = wnd->GetScrollLimit(SB_HORZ);
			
				obj.need_refresh = FALSE;
				if (max > 0 && (obj.pWnd?(obj.pWnd->GetScrollPos(SB_HORZ)> max - diff):(wnd->GetScrollPos(SB_HORZ)) > max - diff))
				{
			...
			if(obj.pPic)
				obj.pPic->SetRect(newrc.left, newrc.top, newrc.Width(), newrc.Height());
			else if(obj.pWnd)
				obj.pWnd->SetWindowPos(NULL,newrc.left, newrc.top,newrc.Width(), newrc.Height(), flags);
			else
				DeferWindowPos(hdwp, obj.hwnd, NULL, newrc.left, newrc.top,newrc.Width(), newrc.Height(), flags);
				//::SetWindowPos(obj.hwnd,NULL,newrc.left, newrc.top,newrc.Width(), newrc.Height(), flags);
		}
	}
...
void CResizableDialog::AddAnchor(CWnd *wnd, CSize tl_type, CSize br_type)
{
	ASSERT(tl_type != NOANCHOR);
	...
	CString st;
	GetClassName(wnd->GetSafeHwnd(), st.GetBufferSetLength(MAX_PATH), MAX_PATH);
	st.ReleaseBuffer();
	st.MakeUpper();
	...
	if (st == "BUTTON")
	{
		DWORD style = GetWindowLong(wnd->GetSafeHwnd(), GWL_STYLE);
		if (style & BS_GROUPBOX)
			SetWindowLong(wnd->GetSafeHwnd(),GWL_STYLE, style | WS_CLIPSIBLINGS);
	}
...
void CResizableDialog::AddAnchor(CPictureObj *pic, CSize tl_type, CSize br_type)
{
	ASSERT(pic != NULL);
	ASSERT(tl_type != NOANCHOR);
	...
	// add to the list
	Layout obj(pic, tl_type, tl_margin,	br_type, br_margin, hscroll, refresh);
	int count = m_arrLayout.GetSize();
	for(int i = 0;i < count; i++)
	{
		Layout pobj = m_arrLayout[i];
		if(pobj.pPic != NULL && pobj.pPic == obj.pPic)
		{
			m_arrLayout.RemoveAt(i);
			break;
		}
...
void CResizableDialog::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized) 
{
	CDialog::OnActivate(nState, pWndOther, bMinimized);

     // Solve multi monitor problem
	//return;
	// 2007-10-20 OZ: Don't fix With only

	// Bug:
	// Application UI disappears.
	// User still online and can hear sounds and receive messages,
	// but there is no way to initiate dialog or see contact lists.
	// Fix:
	if(nState != WA_INACTIVE && !IsIconic())
	{
		CRect r, rd, ri;
		GetWindowRect(&r);
		SystemParametersInfo(SPI_GETWORKAREA, 0, (LPRECT)rd, 0);
		if(!ri.IntersectRect(&rd, &r))
		{
			//Beep(1000,50);
			// 2007-10-20 OZ:
			//if(r.right > rd.right)
