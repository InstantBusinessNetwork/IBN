...
BOOL CCoolMenuManager::LoadToolbar(UINT nIDToolbar)
{
	...
	if (m_szBitmap.cx==0) {
		...
		VERIFY(m_ilButtons.Create(sz.cx, sz.cy, ILC_MASK|ILC_COLOR24, 0, 10));
...
BOOL CCoolMenuManager::LoadToolbar(const CBitmap &Image, TOOLBARDATA *pTbd, COLORREF TransparentColor)
{
	// OK, I have the bitmap and toolbar. 
	...
	if (m_szBitmap.cx==0) 
	{
		...
		// button sizes different -- oops
		TRACE(_T("*** Toolbar Custom button size differs!\n"));
		return FALSE;
	}
	...
	//
	int iNextImage = m_ilButtons.GetImageCount();
	m_ilButtons.Add((CBitmap*)&Image, TransparentColor);
	for (int i = 0; i < pTbd->wItemCount; i++) {
		UINT nID = pTbd->items[i];
		if (nID > 0) {
			if (GetButtonIndex(nID) >= 0) {
								   ...
								   UINT nIndex, BOOL bSysMenu, BOOL bShowButtons)
{
	...
	{	// loop over each item in menu
		...
		BOOL bRetValue = ::GetMenuItemInfo(pMenu->GetSafeHmenu(), i, TRUE, &info);
		...
		if (bShowButtons) 
		{
			...
			if (!(info.fType & MFT_OWNERDRAW)) 
			{
				...
				pmd->text = reinterpret_cast<LPTSTR>(info.dwTypeData);			// copy menu item string
			}
			...
			if (info.fType & MFT_OWNERDRAW) {	// if ownerdraw:
				...
				sItemName = reinterpret_cast<LPTSTR>(info.dwTypeData);		//   use name from MENUITEMINFO
			...
			if (info.fMask & MIIM_TYPE) {
				...
				_tcsncpy(itemname, sItemName, info.cch);
				info.dwTypeData = itemname;
			}
		}
...
//
LONG CCoolMenuManager::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu)
{
	ASSERT_VALID(pMenu);
