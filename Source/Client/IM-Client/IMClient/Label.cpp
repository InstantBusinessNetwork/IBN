...
CLabel& CLabel::SetFontSize(int nSize)
{
	...
	//int lfHeight = -MulDiv(nSize, GetDeviceCaps(GetDesktopWindow()->GetDC()->GetSafeHdc(), LOGPIXELSY), 72);

	cf.CreatePointFont(nSize*100, m_lf.lfFaceName);
	cf.GetLogFont(&lf);

	m_lf.lfHeight = nSize>0?-nSize:nSize;
...
{	
	...
	LOGFONT	lgpFont	=	{0};
	pFont->GetLogFont(&lgpFont);

	m_lf.lfCharSet = byCharSet?byCharSet:lgpFont.lfCharSet;
