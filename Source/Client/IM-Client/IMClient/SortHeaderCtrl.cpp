...
#define	GetWindowThemeFuncName	_T("GetWindowTheme")
...
void CSortHeaderCtrl::SetSortArrow( const int iSortColumn, const BOOL bSortAscending )
{
	...
	// Step 1. Check XP Style [8/29/2002]
	//static HINSTANCE	hUxThemeInst	=	LoadLibrary(_T("UxTheme.dll"));

	if(IsWinXPOrLate())
	{
		int HeadersCount = GetItemCount();
		for(int iHeaderItemIndex = 0;iHeaderItemIndex<HeadersCount;iHeaderItemIndex++)
		{
			HD_ITEM hditem;
			...
			if(iHeaderItemIndex==iSortColumn)
				hditem.fmt |= (bSortAscending ? HDF_SORTUP:HDF_SORTDOWN);

			VERIFY( SetItem( iHeaderItemIndex, &hditem ) );
		}
	}
	else
	{
		// Step 2. Style For Old Windows [8/29/2002]
