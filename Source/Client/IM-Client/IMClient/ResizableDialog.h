...
private:
	...
	public:
		...
		Layout(CWnd* hw, SIZE tl_t, SIZE tl_m, 
			SIZE br_t, SIZE br_m, BOOL hscroll, BOOL refresh)
		{
			hwnd = hw->GetSafeHwnd();
			pWnd = hw;
		...
		Layout(CPictureObj* hw, SIZE tl_t, SIZE tl_m, 
			SIZE br_t, SIZE br_m, BOOL hscroll, BOOL refresh)
		{
			hwnd = NULL;
			pWnd = NULL;
...
//protected:
public:
	void AddAnchor(CPictureObj* pic, CSize tl_type, CSize br_type = NOANCHOR);
	void AddAnchor(CWnd* wnd, CSize tl_type, CSize br_type);
