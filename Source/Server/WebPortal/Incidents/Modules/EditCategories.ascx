<%@ Control Language="c#" Inherits="Mediachase.UI.Web.Incidents.Modules.EditCategories" Codebehind="EditCategories.ascx.cs" %>
...
<table class="text" cellspacing="6" cellpadding="0" width="100%" border="0" style="margin-top:0">
	<tr id="trCategories" runat="server">
		<td>
			<div class="ibn-label"><%= LocRM2.GetString("category")%>:</div>
	...
	<tr id="trIssCategories" runat="server">
		<td>
			<div class="ibn-label"><%= LocRM.GetString("IncidentCategory")%>:</div>
			<div style="height:100px; overflow-y: auto; border: 1px solid black; margin-top:5px">
				<asp:DataGrid id="grdIncidentCategories" runat="server" allowsorting="False" allowpaging="False" width="100%" autogeneratecolumns="False" borderwidth="0" gridlines="None" cellpadding="0" CellSpacing="0" ShowFooter="False" ShowHeader="False">
					<columns>
						<asp:boundcolumn visible="false" datafield="CategoryId"></asp:boundcolumn>
						<asp:templatecolumn itemstyle-cssclass="text">
							<itemtemplate>
								<asp:checkbox runat="server" id="Checkbox1" Text='<%# DataBinder.Eval(Container.DataItem, "CategoryName")%>' Checked='<%# SelectedIncidentCategories.Contains((int)DataBinder.Eval(Container.DataItem, "CategoryId"))%>'></asp:checkbox>
							</itemtemplate>
						</asp:templatecolumn>
					</columns>
				</asp:DataGrid>
			...
			<div align="right">
				<button id="btnAddIncidentCategory" runat="server" style="border:1px;padding:0;height:20px;width:22px;background-color:transparent" type="button"><IMG height="20" src="../layouts/images/icons/dictionary_edit.gif" width="22" border="0"></button>
			</div>
		</td>
	</tr>
	<tr>
