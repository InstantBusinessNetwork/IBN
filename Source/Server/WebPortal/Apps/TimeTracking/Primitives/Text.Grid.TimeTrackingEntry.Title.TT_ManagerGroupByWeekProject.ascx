<%@ Control Language="C#" AutoEventWireup="true" ClassName="Mediachase.Ibn.Web.UI.TimeTracking.Primitives.Text_Grid_TimeTrackingEntry_Title_TT_ManagerGroupByWeekProject" Inherits="Mediachase.Ibn.Web.UI.Controls.Util.BaseType" %>
				...
				if (DataItem.Properties["MetaViewGroupByType"].OriginalValue != null)
				{
					...
					else if (DataItem.Properties["MetaViewGroupByType"].OriginalValue.ToString() == MetaViewGroupByType.Secondary.ToString())
					{
						...
						if (DataItem.Properties["ParentBlockId"].OriginalValue != null)
						{
							...
							//StateMachineService sms = ((Mediachase.IbnNext.TimeTracking.TimeTrackingBlock)ttb).GetService<StateMachineService>();
