<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="FormDocumentDesigner.ascx.cs" Inherits="Mediachase.Ibn.Web.UI.MetaUI.FormDocumentDesigner" %>
<%@ Reference Control="~/Apps/MetaUI/Toolbar/MetaToolbar.ascx" %>
<%@ Register TagPrefix="ibn2" TagName="MetaToolbar" Src="~/Apps/MetaUI/Toolbar/MetaToolbar.ascx" %>
...
<link type="text/css" rel="Stylesheet" href='<%=ResolveUrl("~/styles/IbnFramework/Theme.css") %>' />
<style type="text/css">
	body{
...
<tr>
	<td class="filter" style="padding: 9px 7px 3px 15px;" colspan="2">
		<table width="100%" cellspacing="0" cellpadding="7" border="0" class="ibn-propertysheet" style="table-layout:fixed">
		  <tr>
			<td style="width:120px;"><b><asp:Literal ID="TableLabel" runat="server" Text='<%$Resources : IbnFramework.MetaForm, Table %>' />:</b></td>
			<td style="width:250px;">
				<asp:Label ID="lblTableName" runat="server"></asp:Label>
			</td>
			<td style="width:80px;"><b><asp:Literal ID="Literal6" runat="server" Text='<%$Resources : IbnFramework.MetaForm, FormName %>' />:</b></td>
			<td>
...
<tr>
	...
	<td class="rightCustomTd text" valign="top" id="tdRight" runat="server">
		<div class="rightCustomHeader"><asp:Literal ID="Literal1" runat="server" Text='<%$Resources : IbnFramework.MetaForm, CommonTasks %>' /></div>
		<div class="rightCustomPanel ibn-propertysheet" style="height:100%">
			<table width="100%" cellspacing="0" cellpadding="2" border="0">
			  <tr>
				<td><b><asp:Literal ID="Literal2" runat="server" Text='<%$Resources : IbnFramework.MetaForm, Table %>' />:</b></td>
				<td colspan="2">
			  ...
			  <tr>
				<td><b><asp:Literal ID="Literal3" runat="server" Text='<%$Resources : IbnFramework.MetaForm, FormName %>' />:</b></td>
				<td colspan="2">
			...
			<table cellspacing="0" cellpadding="0" width="100%" id="tblLinks" runat="server">
				<tr>
					<td>
						...
						<hr />
						<b><asp:Literal ID="Literal4" runat="server" Text='<%$Resources : IbnFramework.MetaForm, SelectedElements %>' />:</b>
						<ul class="rightDesigner">
							...
							<li><asp:LinkButton ID="lbRemoveSection" runat="server" Text='<%$Resources : IbnFramework.MetaForm, RemoveSection %>' OnClick="lbRemoveSection_Click"></asp:LinkButton></li>
							<li><asp:LinkButton ID="lbRemoveField" runat="server" Text='<%$Resources : IbnFramework.MetaForm, RemoveField %>' OnClick="lbRemoveField_Click"></asp:LinkButton></li>
						</ul>
						<hr />
						<ul class="rightDesigner">
							<li><asp:LinkButton ID="lbSave" runat="server" Text='<%$Resources : IbnFramework.MetaForm, Save %>' OnClick="lbSave_Click"></asp:LinkButton></li>
							<li><asp:LinkButton ID="lbSaveClose" runat="server" Text='<%$Resources : IbnFramework.MetaForm, SaveClose %>' OnClick="lbSaveClose_Click"></asp:LinkButton></li>
							<li><asp:LinkButton ID="lbReCreate" runat="server" Text='<%$Resources : IbnFramework.MetaForm, RecreateForm %>' OnClick="lbReCreate_Click"></asp:LinkButton></li>
						</ul>
					</td>
				</tr>
			</table>
...
<script type="text/javascript">
	...
	{
		if(confirm('<%=GetGlobalResourceObject("IbnFramework.MetaForm", "RecreateFormWarning").ToString() %>'))
		{
	...
	function onRemoveClick(item)
	{
		...
		{
			if(confirm('<%=GetGlobalResourceObject("IbnFramework.MetaForm", "WarningSection").ToString() %>'))
			{
				BeforeDelete();
		...
		{
			if(confirm('<%=GetGlobalResourceObject("IbnFramework.MetaForm", "WarningField").ToString() %>'))
			{
				BeforeDelete();
	...
	{
		var link = '<%=this.ResolveClientUrl("~/Apps/MetaUI/Pages/Public/FormItemEdit.aspx") %>';
		link = link + '?add=1&uid=';
	...
	{
		var link = '<%=this.ResolveClientUrl("~/Apps/MetaUI/Pages/Public/FormSectionEdit.aspx") %>';
		link = link + '?uid=';
		var obj = document.getElementById('<%=hFieldKey.ClientID %>');
		link = link + obj.value;
		link = link + '&btn=<%=lbAddSection.UniqueID %>';
	...
	{
		var link = '<%=this.ResolveClientUrl("~/Apps/MetaUI/Pages/Public/FormDocumentEdit.aspx") %>';
		link = link + '?uid=';
		var obj = document.getElementById('<%=hFieldKey.ClientID %>');
		link = link + obj.value;
