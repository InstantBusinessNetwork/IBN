...
namespace Mediachase.Ibn.Web.UI
{
	public partial class PickerControl : System.Web.UI.UserControl
	{
		...
		#region prop: InnerDateControl
		/// <summary>
		/// Gets the inner date control.
		/// </summary>
		/// <value>The inner date control.</value>
		public TextBox InnerDateControl
		{
			get
			{
				return txtDate;
		...
		#region prop: InnerTimeControl
		/// <summary>
		/// Gets the inner time control.
		/// </summary>
		/// <value>The inner time control.</value>
		public TextBox InnerTimeControl
		{
			get
			{
				return txtTime;
		...
		public DateTime SelectedDate
		{
			get
			{
				...
				else
				{
					...
					//TIME
					#region Time
					//Inogda ot date controla prihodit data + vrema
					if (retVal.Hour != 0)
						retVal = retVal.AddHours(-retVal.Hour);
		...
		#region prop: RangeValidator
		public bool RangeValidator
		{
			get
			{
				if (ViewState[this.ID + "_RangeValidator"] != null)
					return (bool)ViewState[this.ID + "_RangeValidator"];
				else
					return false;
			}
			set
			{
				ViewState[this.ID + "_RangeValidator"] = value;
			}
		}
		#endregion

		#region prop: RangeMinimum
		public DateTime RangeMinimum
		{
			get
			{
				if (ViewState[this.ID + "_RangeMinimum"] != null)
					return (DateTime)ViewState[this.ID + "_RangeMinimum"];
				else
					return DateTime.MinValue;
			}
			set
			{
				ViewState[this.ID + "_RangeMinimum"] = value;
			}
		}
		#endregion

		#region prop: RangeMaximum
		public DateTime RangeMaximum
		{
			get
			{
				if (ViewState[this.ID + "_RangeMaximum"] != null)
					return (DateTime)ViewState[this.ID + "_RangeMaximum"];
				else
					return DateTime.MaxValue;
			}
			set
			{
				ViewState[this.ID + "_RangeMaximum"] = value;
			}
		}
		#endregion

		#region prop: TabIndex
		/// <summary>
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			...
			if (AutoPostBack)
			{
				...
				calendarButtonExtender.UpdateElementId = hfUpdateCalendar.ClientID;
				tpExt.UpdateElementId = txtTime.UniqueID;
			...
			{
				rvDate.Enabled = true;
				rvDate.ErrorMessage = String.Format("{0} - {1}", RangeMinimum.ToShortDateString(), RangeMaximum.ToShortDateString());
				rvDate.Display = ValidatorDisplay.Dynamic;
				rvDate.MinimumValue = RangeMinimum.ToShortDateString();
				rvDate.MaximumValue = RangeMaximum.ToShortDateString();
		...
		#region hfUpdateCalendar_ValueChanged
		/// <summary>
		/// Handles the ValueChanged event of the hfUpdateCalendar control.
		/// </summary>
		...
		#region GetMonthViewLimits
		/// <summary>
		/// Gets the month view limits.
		/// </summary>
