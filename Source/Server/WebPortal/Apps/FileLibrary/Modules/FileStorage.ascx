<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="FileStorage.ascx.cs" Inherits="Mediachase.Ibn.Web.UI.FileLibrary.Modules.FileStorage" %>
<%@ Reference Control="~/Apps/Common/Design/BlockHeader2.ascx" %>
<%@ Reference Control="~/Apps/MetaUI/Toolbar/MetaToolbar.ascx" %>
<%@ Reference Control="~/Apps/HelpDeskManagement/Modules/MCGrid.ascx" %>
<%@ Register TagPrefix="mc" TagName="BlockHeader2" Src="~/Apps/Common/Design/BlockHeader2.ascx" %>
<%@ Register TagPrefix="mc" TagName="MCGrid" Src="~/Apps/HelpDeskManagement/Modules/MCGrid.ascx" %>
<%@ Register TagPrefix="mc" TagName="MetaToolbar" Src="~/Apps/MetaUI/Toolbar/MetaToolbar.ascx" %>
<%@ Register Namespace="Mediachase.Ibn.Web.UI.WebControls" Assembly="Mediachase.Ibn.Web.UI.WebControls" TagPrefix="mcext" %>
<link rel="stylesheet" type="text/css" href='<%= Mediachase.Ibn.Web.UI.WebControls.McScriptLoader.Current.GetScriptUrl("~/Styles/IbnFramework/grid.css", this.Page) %>' />
<link rel="stylesheet" type="text/css" href='<%= ResolveClientUrl("~/Styles/Shell/mainLeftTemplate.css") %>' />
<script type="text/javascript">
var resizeFlag = false;
function LayoutResizeHandler(sender, eventArgs) {
	var obj = document.getElementById('divLeftContainer');
	var objPanel = document.getElementById('divLeftPanel');

	if (obj && objPanel) {
		var iheight = obj.offsetHeight;
		objPanel.style.height = iheight + "px";
		//alert(iheight);
	}
}

function OpenWindow(query,w,h,scroll)
{
	var l = (screen.width - w) / 2;
	var t = (screen.height - h) / 2;
	
	winprops = 'resizable=0, height='+h+',width='+w+',top='+t+',left='+l;
	if (scroll) winprops+=',scrollbars=1';
	var f = window.open(query, "_blank", winprops);
}

function File_Storage_GetSelectedItems(params)
{
	var obj = $find('<%=grdMain.GridClientContainerId %>');
	if(obj != null)
	{
		var ss = "";
		var selected = obj.getCheckedCollection();
		var mas = selected.split(";");
		for(var i=0; i<mas.length;i++)
		{
			var elem = mas[i];
			var elemMas = elem.split("::");
			if(elemMas[0] == "2")
				ss += elemMas[1] + ",";
		}
		return ss;
	}
	else 
		return "";
}

function FileStorage_ExistSelectedItems(params)
{
	var obj = $find('<%=grdMain.GridClientContainerId %>');
	if(obj != null)
	{
		if(obj.isChecked())
			return true;
	}
	return false;
}

function FileStorage_ExistSelectedFiles(params)
{
	var obj = $find('<%=grdMain.GridClientContainerId %>');
	if(obj != null)
	{
		var selected = obj.getCheckedCollection();
		if(selected.length == 0)
			return false;
		var mas = selected.split(";");
		for(var i=0; i<mas.length;i++)
		{
			var elem = mas[i];
			var elemMas = elem.split("::");
			if(elemMas[0] == "2")
				return true;
		}
	}
	return false;
}
</script>
<style type="text/css">
	.hundred
	{
		height: 100%;
		width: 100%;
		position: absolute;
	}
	.innerArea
	{
		background-color: #D6E8FF;
		border-top:1px solid #A6A6A6;
		border-left: 1px solid #6B92CE;
		border-right: 1px solid #6B92CE;
		border-bottom: 1px solid #6B92CE;
	}
	.innerContentArea
	{
		background-color: White;
		border:1px solid #BBD4F6;
	}
</style>
<mcext:McDock ID="DockMainTop" runat="server" Anchor="top" EnableSplitter="False" DefaultSize="21">
	<DockItems>
		<table cellspacing="0" cellpadding="0" border="0" width="100%" class="filter">
			<tr>
				<td class="bottomBorder">
					<mc:BlockHeader2 runat="server" ID="BlockHeaderMain" />
				</td>
			</tr>
		</table>
	</DockItems>
</mcext:McDock>

<mcext:McDock ID="DockTop" runat="server" Anchor="top" EnableSplitter="False" DefaultSize="51">
	<DockItems>
		<asp:UpdatePanel ID="upToolbarSearch" runat="server" UpdateMode="Conditional">
			<ContentTemplate>
				<table cellspacing="0" cellpadding="0" border="0" width="100%" class="filter ibn-propertysheet">
					<tr>
						<td style="padding:7px;">
							<asp:label id="lblPathTitle" CssClass="text" runat="server"></asp:label>:&nbsp;&nbsp;
							<asp:label id="lblPath" runat="server" CssClass="ibn-nav"></asp:label>
						</td>
					</tr>
					<tr>
						<td style="padding-left: 5px; padding-right: 5px;">
							<div class="noBottomBorder">
								<mc:MetaToolbar runat="server" ID="MainMetaToolbar" GridId="grdMain" />
							</div>		
						</td>
					</tr>	
				</table>
			</ContentTemplate>
		</asp:UpdatePanel>
	</DockItems>
</mcext:McDock>	
<mcext:McDock ID="DockLeft" InnerCssClass="hundred" runat="server" Anchor="Left" EnableSplitter="false" DefaultSize="250">
	<DockItems>
		<div style="height: 100%; width: 100%; position: relative;" class="filter">
			<div style="position: absolute; left:5px; bottom: 3px; top: 0px; right:0px;" class="innerArea">
			<asp:UpdatePanel ID="IssListLeftPanel" runat="server" UpdateMode="Conditional">
				<ContentTemplate>
					<div style="position:absolute;left:9px;right:9px;top:9px;bottom:9px;" class="innerContentArea" id="divLeftContainer">
						<div style="overflow-y:auto;height:500px;" id="divLeftPanel">
							<mcext:JsTreePanel AutoHeight="true" AutoScroll="true" id="jsTreeExt" runat="server" FillToContainer="true" IconCss="iconStdCls" NodeTextCss="nodeCls"></mcext:JsTreePanel>	
						</div>
					</div>
				</ContentTemplate>
			</asp:UpdatePanel>
			</div>
		</div>
	</DockItems>
</mcext:McDock>
<table style="margin-top:0px; padding-top: 0px; table-layout: fixed;" cellspacing="0" cellpadding="0" border="0" width="100%" class="ibn-propertysheet">
	<tr>
		<td style="padding-left: 5px;" class="filter">
			<asp:UpdatePanel ID="grdMainPanel" runat="server" UpdateMode="Conditional">
				<ContentTemplate>
					<mc:MCGrid ID="grdMain" runat="server" WeightSort="true" />	
				</ContentTemplate>
			</asp:UpdatePanel>
		</td>
	</tr>
</table>