<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ProjectList.ascx.cs" Inherits="Mediachase.Ibn.Web.UI.ProjectManagement.Modules.ProjectList" %>
...
function LayoutResizeHandler(sender, eventArgs)
{
	...
	var labelDiv = document.getElementById('divLeftHeaderContainer');
	var txtGroupDiv = document.getElementById('<%= divLeftSearchContainer.ClientID%>');
	var objPanel = document.getElementById('divLeftPanel');

	if(obj && labelDiv && objPanel)
	{
		var iheight = obj.offsetHeight - labelDiv.offsetHeight - 10;
							...
							<table cellpadding="5" cellspacing="0" width="100%" border="0" class="ibn-propertysheet" style="table-layout:fixed;">
								...
								<tr>
									...
									<td align="right" style="padding:5px 5px 0px 0px;">
										<nobr><%=GetGlobalResourceObject("IbnFramework.Project", "ViewFieldSet").ToString()%>: <mc2:IndentedDropDownList ID="ddFilters" runat="server" Width="260px" onchange="OnChangeView(this)"></mc2:IndentedDropDownList>&nbsp;
				...
				<div style="position:absolute;left:9px;right:9px;top:40px;bottom:9px;" class="innerContentArea" id="divLeftContainer">
					<div style="padding:5px 1px 5px 5px;">
						<asp:UpdatePanel ID="upLeftArea" runat="server" UpdateMode="Conditional">
							<ContentTemplate>
								<div style="padding:0px 0px 5px 3px;" id="divLeftHeaderContainer">
									<b><asp:Label ID="lblGroupName" runat="server" CssClass="text"></asp:Label>:</b>
								</div>
								<div style="padding:0px 0px 5px 3px;" id="divLeftSearchContainer" runat="server">
...
<asp:DataGrid ID="dgExport" runat="server" AutoGenerateColumns="False" AllowPaging="False"
	...
	<Columns>
		<asp:TemplateColumn>
			<ItemTemplate>
				<%# Mediachase.Ibn.Web.UI.CHelper.GetProjectNum((int)Eval("ProjectId"), Eval("ProjectCode").ToString())%>
			</ItemTemplate>
		</asp:TemplateColumn>
