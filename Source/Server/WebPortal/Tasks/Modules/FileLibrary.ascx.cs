namespace Mediachase.UI.Web.Tasks.Modules
{
    ...
    protected UserLightPropertyCollection pc = Security.CurrentUser.Properties;
		private string pcKey = "TKFS_CurrentTab";
		...
		{
			// Add Tabs
      string sLinkBegin = (Security.CurrentUser.IsExternal) ? "ExternalTask.aspx" : "TaskView.aspx";

      string link = String.Format("{1}?TaskId={0}&Tab=FileLibrary&SubTab=0", TaskId, sLinkBegin);
      ...
      {
        link = String.Format("{1}?TaskId={0}&Tab=FileLibrary&SubTab=1", TaskId, sLinkBegin);
        blockControl.AddTab("1", LocRM.GetString("tFileList"), link, "../FileStorage/Modules/FilesList.ascx");

        link = String.Format("{1}?TaskId={0}&Tab=FileLibrary&SubTab=2", TaskId, sLinkBegin);
