...
namespace Mediachase.UI.Web.Projects.Modules
{
	...
	/// </summary>
	public partial  class LatestMyUpdates : System.Web.UI.UserControl
	{
		private UserLightPropertyCollection pc =  Security.CurrentUser.Properties;
		protected ResourceManager LocRM = new ResourceManager("Mediachase.UI.Web.App_GlobalResources.Workspace.Resources.strLatestUpdates", typeof(LatestMyUpdates).Assembly);
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (pc["ProjectUSetup_MyUpdatesLastDays"] != null)
				NumOfDays = int.Parse(pc["ProjectUSetup_MyUpdatesLastDays"]);
		...
		{
			DataTable dt = Incident.GetListIncidentsUpdatedByUserDataTable(NumOfDays, ProjectID);
			ViewState["IncidentsRowCount"] = dt.Rows.Count;
			
			if (dt.Rows.Count == 0 || 
				(pc["ProjectUSetup_ShowMyIncidents"]!=null && pc["ProjectUSetup_ShowMyIncidents"]=="False"))
			{
				sep2.Visible = false;
				Pan2.Visible = false;
			}
			...
			dgIncidents.Columns[1].HeaderText = LocRM.GetString("Modify");
		...
		{
			DataTable dt = Document.GetListDocumentsUpdatedByUserDataTable(NumOfDays, ProjectID);
			ViewState["DocumentsRowCount"] = dt.Rows.Count;
			
			if (dt.Rows.Count == 0 || 
				(pc["ProjectUSetup_ShowMyDocuments"]!=null && pc["ProjectUSetup_ShowMyDocuments"]=="False"))
			{
				sep3.Visible = false;
				Pan3.Visible = false;
			}
		...
		{
			DataTable dt = CalendarEntry.GetListEventsUpdatedByUserDataTable(NumOfDays, ProjectID);
			ViewState["EventsRowCount"] = dt.Rows.Count;
			
			if (dt.Rows.Count == 0 || 
				(pc["ProjectUSetup_ShowMyEvents"]!=null && pc["ProjectUSetup_ShowMyEvents"]=="False"))
			{
				sep4.Visible = false;
				Pan4.Visible = false;
			}
			...
			dgEvents.Columns[1].HeaderText = LocRM.GetString("Modify");
		...
		{
			DataTable dt = ToDo.GetListToDoAndTasksUpdatedByUserDataTable(NumOfDays, ProjectID);
			ViewState["TasksRowCount"] = dt.Rows.Count;
			
			if (dt.Rows.Count == 0 || 
				(pc["ProjectUSetup_ShowMyTasks"]!=null && pc["ProjectUSetup_ShowMyTasks"]=="False"))
			{
				sep5.Visible = false;
				Pan5.Visible = false;
			}
			...
			dgTasks.Columns[1].HeaderText = LocRM.GetString("Modify");
		...
		{
			tbHeader.Title = LocRM.GetString("tbMyTitle");
		...
		{
			pc["ProjectMyLatestUpdates_sep2"] = sep2.IsMinimized.ToString();
			pc["ProjectMyLatestUpdates_sep3"] = sep3.IsMinimized.ToString();
			pc["ProjectMyLatestUpdates_sep4"] = sep4.IsMinimized.ToString();
			pc["ProjectMyLatestUpdates_sep5"] = sep5.IsMinimized.ToString();
		} 
		#endregion
		...
		{
			if (pc["ProjectMyLatestUpdates_sep2"] != null) sep2.IsMinimized = bool.Parse(pc["ProjectMyLatestUpdates_sep2"]);
			if (pc["ProjectMyLatestUpdates_sep3"] != null) sep3.IsMinimized = bool.Parse(pc["ProjectMyLatestUpdates_sep3"]);
			if (pc["ProjectMyLatestUpdates_sep4"] != null) sep4.IsMinimized = bool.Parse(pc["ProjectMyLatestUpdates_sep4"]);
			if (pc["ProjectMyLatestUpdates_sep5"] != null) sep5.IsMinimized = bool.Parse(pc["ProjectMyLatestUpdates_sep5"]);
		} 
		#endregion
