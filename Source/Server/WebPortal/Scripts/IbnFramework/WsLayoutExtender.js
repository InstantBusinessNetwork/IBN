...
Ibn.WsLayoutExtender = function(element) {
    ...
    var itemsArray = null;
    var disablePersonolization = null;
...
Ibn.WsLayoutExtender.prototype =
{
    ...
    get_disablePersonolization: function() {
        return this.disablePersonolization;
    },
    set_disablePersonolization: function(value) {
        this.disablePersonolization = value;
    },
    ...
    initialize: function() {
        ...
        if (this.disablePersonolization == false) {
    ...
    onWindowResize: function(e) {
        if (this._element && document.body.offsetHeight - this._element.offsetTop > 0) {
            ...
            if (typeof (this.itemsArray) === 'undefined') {
                this.itemsArray = Sys.Serialization.JavaScriptSerializer.deserialize(this.jsonItems);
            }

            if (document.all && typeof (this.itemsArray) !== 'undefined')//resize width for ie 7
            {
                ...
                {
//                    for (var i = 0; i < this.itemsArray.length; i++) {
                        var innerColumnObj = document.getElementById(this.itemsArray[this.itemsArray.length - 1].clientId);
                        if (innerColumnObj) {
                            innerColumnObj.style.width = innerColumnObj.offsetWidth - 20 + 'px';
                        }
//                    }
                }
            }
    ...
    attachEventToButton: function(obj, iCol, iItem, iButton) {
        if (obj) 
        {
            if (this.disablePersonolization === true && obj.IsSystem === false)
                return;
    ...
    attachHandlers: function(clearHandlers) {
        for (var i = 0; i < this.itemsArray.length; i++) {
            if (typeof (this.itemsArray[i].items) == "undefined")
                this.itemsArray[i].items = new Array();
            //attach events:
    ...
    initLayout: function() {
        ...
        this.itemsArray = Sys.Serialization.JavaScriptSerializer.deserialize(this.jsonItems.replace(/True/g, 'true').replace(/False/g, 'false'));
        if (this.disablePersonolization == false) 
        {
    ...
    mouseCoords: function(ev) {
        if ((ev.pageX || ev.pageY) && this._element) {
            return { x: ev.pageX - this._element.offsetLeft - document.body.clientLeft, y: ev.pageY - this._element.offsetTop - document.body.clientTop };
        }
        if (this._element)
            return {
                x: ev.clientX + this._element.scrollLeft - this._element.offsetLeft - document.body.clientLeft, //- document.body.clientLeft,
                y: ev.clientY + this._element.scrollTop - this._element.offsetTop - document.body.clientTop//- document.body.clientTop
            };
        else
            return { x: 0, y: 0 };
    },
    ...
    mouseMove: function(ev) {
        ...
        */
        ...
        if (this.curTarget) {
            ...
            this.dragHelper.style.top = mousePos.y + this._element.offsetTop /*- this.mouseOffset.y */ + 'px';
            this.dragHelper.style.left = mousePos.x + this._element.offsetLeft/* - this.mouseOffset.x */ + 'px';
        ...
        if (this.curTarget) {
            ...
            if (fictiveNode) {
                if (fictiveNode.nextSibling == null)
                    this.newItemIndex++;
                var _counter = 0;
                for (var i = 0; i < fictiveNode.parentNode.childNodes.length; i++) {
                    if (fictiveNode.parentNode.childNodes[i].nodeType != 1)
                        continue;
                    if (fictiveNode.parentNode.childNodes[i].id === fictiveNode.id) {
                        this.newItemIndex = _counter;
                    }
                    _counter++;
                }
            ...
            if (this.newItemIndex > this.itemsArray[this.newColumnIndex].items.length)
                this.newItemIndex = this.itemsArray[this.newColumnIndex].items.length;
            ...
            if (this.oldColumnIndex == this.newColumnIndex) {
                ...
                if (this.newItemIndex != 0 && this.newItemIndex > this.oldItemIndex)//this.newItemIndex < this.itemsArray[this.newColumnIndex].items.length - 1 && 
                    this.newItemIndex++;
            ...
            Mediachase.Ibn.Web.UI.WebServices.LayoutHandler.OrderChange(this.getCurrentJson(), this.contextKey, null, Function.createDelegate(this, this.onRequestFailed));
        ...
        for (var i = 0; i < this.itemsArray.length; i++) {
            ...
            for (var j = 0; j < this.itemsArray[i].items.length; j++) {
                ...
                if (this.itemsArray[i].items[j].id.split('_').length > 1)
                    newItem.instanseUid = this.itemsArray[i].items[j].id.split('_')[1];
