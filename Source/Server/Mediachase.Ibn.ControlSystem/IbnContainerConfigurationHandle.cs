...
namespace Mediachase.Ibn.ControlSystem
{
	...
	public class IbnContainerConfigurationHandle: IConfigurationSectionHandler
	{
		...
		#region IbnContainerConfiguration()
		internal IbnContainerConfiguration GetConfig()
		{
			System.Reflection.Assembly asm = this.GetType().Assembly;
			using(Stream stream = asm.GetManifestResourceStream("Mediachase.Ibn.ControlSystem.config.xml"))
			{
				XmlDocument doc = new XmlDocument();
				doc.Load(stream);
				return (IbnContainerConfiguration)Create(null, null, doc.SelectSingleNode("ibnContainers"));
			}
		}
		#endregion
	}
